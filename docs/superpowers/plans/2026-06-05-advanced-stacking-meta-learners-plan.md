# Advanced Stacking Meta-Learners Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement 10 ensembling meta-learner configurations—including ElasticNet regularisation, highly regularised shallow GBDTs, and direct Nelder-Mead out-of-fold blending weight optimization—to optimize combination validation score.

**Architecture:** We will create a programmatic update script to refactor cells `imports_code`, `stacking_meta_md`, and `stacking_meta_code` in `notebooks/03_model_tuning_and_ensemble.ipynb` to compare configurations side-by-side using Stratified 5-Fold CV, dynamically selecting the highest-scoring meta-learner for final threshold scaling and inference.

**Tech Stack:** Python, scikit-learn, LightGBM, XGBoost, SciPy

---

### Task 1: Create Blender Update Script

**Files:**
- Create: [scratch/update_blender_v12.py](file:///Users/tuanm.nguyen/Documents/kaggle-s6e6-predicting-stellar-class/scratch/update_blender_v12.py)
- Modify: [notebooks/03_model_tuning_and_ensemble.ipynb](file:///Users/tuanm.nguyen/Documents/kaggle-s6e6-predicting-stellar-class/notebooks/03_model_tuning_and_ensemble.ipynb)

- [ ] **Step 1: Write the update script**
  Create `scratch/update_blender_v12.py` in the workspace to programmatically look up and replace cell contents.
...
- [ ] **Step 2: Execute code injection**
  Run: `python3 scratch/update_blender_v12.py`
  Expected: Prints "Successfully updated notebooks/03_model_tuning_and_ensemble.ipynb!"

- [ ] **Step 3: Verify updated notebook formatting**
  Run: `python3 -c "import json; json.load(open('notebooks/03_model_tuning_and_ensemble.ipynb'))"`
  Expected: Successful parse without any JSON syntax error.

- [ ] **Step 4: Commit changes to Git**
  Run:
  ```bash
  git add scratch/update_blender_v12.py notebooks/03_model_tuning_and_ensemble.ipynb
  git commit -m "feat: add advanced stacking meta-learners to ensembling pipeline"
  ```

---

### Task 2: Push and Execute Ensembling Upgrade on Kaggle

**Files:**
- Modify: [notebooks/kernel-metadata.json](file:///Users/tuanm.nguyen/Documents/kaggle-s6e6-predicting-stellar-class/notebooks/kernel-metadata.json)

- [ ] **Step 1: Set push metadata to ensembling configuration**
  Run: `python3 scratch/prepare_ensemble_v2.py`
  Expected: Prints updated metadata config details for ensembling.

- [ ] **Step 2: Push ensembling notebook to Kaggle**
  Run: `/Users/tuanm.nguyen/Library/Python/3.9/bin/kaggle kernels push -p notebooks`

- [ ] **Step 3: Monitor ensembling execution**
  Run: `python3 scratch/poll_ensemble.py`
  Expected: Status completes in ~5-10 minutes.
